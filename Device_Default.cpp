...
bool KDevice::Init_DefaultData()
{
	ResourceManager<Blend>::Create(L"AlphaBlend");
	/************************* Material _ Line **************************/
	//KVector4 NewLineVertex[4] = {};

	//NewLineVertex[0] = KVector4(-0.5f, 0.5f, 0.0f, 1.0f);
	//NewLineVertex[1] = KVector4(0.5f, 0.5f, 0.0f, 1.0f);
	//NewLineVertex[2] = KVector4(-0.5f, -0.5f, 0.0f, 1.0f);
	//NewLineVertex[3] = KVector4(0.5f, -0.5f, 0.0f, 1.0f);

	//// 기존 렉트
	//WORD LineRect_Index[5] = { 0, 1, 3, 2, 0 };
	...
	LineMaterial->Set_VertexShader(L"LINE_VERTEX");
	LineMaterial->Set_PixelShader(L"LINE_PIXEL");
	...
	ColorMaterial->Set_VertexShader(L"VSCOLOR");
	ColorMaterial->Set_PixelShader(L"PSCOLOR");
	...
	TexMaterial->Set_VertexShader(L"VSTEX");
	TexMaterial->Set_PixelShader(L"PSTEX");
	...
	ImgMaterial->Set_VertexShader(L"VSIMG");
	ImgMaterial->Set_PixelShader(L"PSIMG");
	ImgMaterial->Set_Blend(L"AlphaBlend");


	Core_Class::MainDevice().Create_ConstBuf<KMatrix>(L"TRANS", D3D11_USAGE_DYNAMIC, 1);
	Core_Class::MainDevice().Create_ConstBuf<KVector4>(L"MULTI", D3D11_USAGE_DYNAMIC, 2);

	// 아니... 자료형 넘길 때 (짝수 * 4)만 되는 거 실화?? - 여튼 테두리 쉐이더
	Core_Class::MainDevice().Create_ConstBuf<KBorder>(L"BORDER", D3D11_USAGE_DYNAMIC, 3);
	Core_Class::MainDevice().Create_ConstBuf<KVector4>(L"POLY", D3D11_USAGE_DYNAMIC, 4);
