...
bool Collision::collision_check(Collision* _Other)
{
	...
	KPtr<KRigid_2D> T1 = Test1->Get_Component<KRigid_2D>();
	KPtr<KRigid_2D> T2 = Test2->Get_Component<KRigid_2D>();

	if (nullptr != T1 && b2BodyType::b2_staticBody == T1->Get_Type())
	{
		return false;
	}
	if (nullptr != T2 && b2BodyType::b2_staticBody == T2->Get_Type())
	{
		return false;
	}
