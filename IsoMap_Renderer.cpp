...
void IsoMap_Renderer::Render(const KMatrix& _VP)
{
	...
	for (; S_IsoTile != E_IsoTile; ++S_IsoTile)
	{
		...
		window()->device().Set_ConstBuffer<KMatrix>(L"TRANS", Mat.TransPose_Referance(), Shader::SHADER_TYPE::ST_VS);
		window()->device().Set_ConstBuffer<KBorder>(L"BORDER", NewBorder, Shader::SHADER_TYPE::ST_PS);
		window()->device().Set_ConstBuffer<KVector4>(L"MULTI", m_Image->uv(S_IsoTile->second->m_Index), Shader::SHADER_TYPE::ST_PS);
